...
{
    /**
     * Constructor for objects of class FilppedWarholFilter.
        ...
        for(int y = 0; y < height; y++) {
            for(int x = 0; x < width; x++) {
                ...
                if (x < halfWidth && y < halfHeight){
                    ...
                    pix = original.getPixel((halfWidth - 1 - localx) * 2, localy * 2);
                    pix = new Color(pix.getRed(), 0, 0);
                }
                else if (x < halfWidth && y >= halfHeight){
                    pix = original.getPixel(localx * 2, (halfHeight - 1 - localy) * 2);
                    pix = new Color(0, pix.getGreen(), 0);
                }
                else{
                    pix = original.getPixel((halfWidth - 1 - localx) * 2, (halfHeight - 1 - localy) * 2);
